namespace Q1.Emulator.Assembler;
...
public class Disassembler
{
    ...
    public static string ParseInstruction(u16 instruction)
    {
        ...
        string opcodeName = Disassembler.GetOpcodeName(op, i);

        if (i == 0 || i == 2)
            return opcodeName;

        if (i == 1)
            return $"{opcodeName} {Disassembler.GetModeName(m1)}";

        if (i == 3)
            return $"{opcodeName} {Disassembler.GetModeName(m1)}, {Disassembler.GetModeName(m2)}";
    ...
    {
        if (Disassembler.ModeLookup.TryGetValue(mode, out string? modeName))
            return modeName;
